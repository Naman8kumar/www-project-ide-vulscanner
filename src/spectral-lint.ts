 ...
 */
export function runSpectralLint(document: vscode.TextDocument, spectralDiagnostics: vscode.DiagnosticCollection) {
    ...
    exec(`npx spectral lint ${filePath} -f json`, (error, stdout, stderr) => {
        ...
        try {
            ...
            vscode.window.showInformationMessage(`Spectral Linting Completed. Found ${diagnostics.length} issue(s).`);
        } catch (parseError) {
